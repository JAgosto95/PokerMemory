...
public abstract class GameLevel implements ActionListener 
{
	...
	protected static ScoreCalculationLabel currentScore;		//The score counter is incremented or decreased after each turn
	protected static Timer turnDownTimer;   					// Timer is used to make a delay
	protected int turnDownDelay = 500;							// Milliseconds to leave cards up before turning them back down (we lowered the amount of time for a fast paced game)
	 ...
	 */
	protected GameLevel(TurnsTakenCounterLabel counterLabel, ScoreCalculationLabel scoreLabel, int cardsToGuess, JFrame mainFrame)
	{
		this.turnsTakenCounter = counterLabel;
		GameLevel.currentScore = scoreLabel;
		this.turnedCardsBuffer= new Vector<Card>(cardsToGuess);
		this.mainFrame = mainFrame;
		GameLevel.turnDownTimer = new Timer(turnDownDelay, this);
		GameLevel.turnDownTimer.setRepeats(false);
